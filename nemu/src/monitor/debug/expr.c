...
static struct rule {
  ...
  {"0|[1-9][0-9]*", TK_NUMBER},                  // dec number
...
int eval(int p, int q, bool* success) {
  ...
  if(p == q) {
    ...
    for(int i = p; i <= q; i++) {
      ...
      if(tokens[i].type == TK_NUMBER || tokens[i].type == TK_HEX || tokens[i].type == TK_REG) {
          ...
          while(operators_stack[operators_index].type != TK_NOTYPE &&
            ...
            if(operators_stack[operators_index].type == ')') {
              ...
              switch(operators_stack[operators_index--].type) {
                case '+':
                  operands_stack[operands_index - 1] = operands_stack[operands_index] + operands_stack[operands_index - 1];
                  operands_index--;
                  break;
                case '-':
                  operands_stack[operands_index - 1] = operands_stack[operands_index] - operands_stack[operands_index - 1];
               ...
               case '/':
                ...
                case TK_GEQ:
                  ...
                  break;
                case TK_NEG:
                  operands_stack[operands_index] = -operands_stack[operands_index];
                  break;
               case TK_DEREF:
                  operands_stack[operands_index] = paddr_read(operands_stack[operands_index], 4);
                  break;
                default:
                  success = false;
                  return 0;
